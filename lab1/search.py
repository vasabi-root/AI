...
class Search:
    ...
    parent: object          # указатель на родителя
    ...
    def __init__(self, state: List[List[int]], parent=None, fringer: List[List]=None, endHash: int=0) -> None:
        self.parent = parent
        if (self.parent == None):
            self.depth = 0
            self.root = self
    ...
    def DFSopen(self, fringer: List[List[object]]) -> None:
        ...
        for row in [self.z_row-1, self.z_row+1]:
            if (0 <= row < self.m):
                ...
                if (not Search.alreadyExists(self.root, hash)):
    ...
    def alreadyExists(root, hash) -> bool:
        ...
        if (node.hash == hash):
            ...
            while (i < l and ans == False):
                ans = Search.alreadyExists(node.children[i], hash)
                i += 1
        return ans
